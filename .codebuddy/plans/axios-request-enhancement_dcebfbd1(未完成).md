---
name: axios-request-enhancement
overview: 增强 axios-request 库：重构四大管理器（Token/Dedupe/Cancel/Retry），实现实例级+私有级配置合并规则，新增核心扩展，并创建 Vue3+Element Plus+UnoCSS 示例项目。
design:
  styleKeywords:
    - Cyberpunk Neon UI
    - Dark Mode
...
todos:
  ...
  - id: refactor-cancel-manager
    ...
    dependencies:
      - implement-config-merger
  - id: implement-manager-factory
    content: 实现 ManagerFactory：创建和缓存私有级管理器实例
    status: pending
    dependencies:
      - refactor-types
...
对现有 `axios-request` 库进行重构和功能增强，实现四大管理器（Token、Retry、Dedupe、Cancel）的完整功能，并新增 Vue3 + Element Plus + UnoCSS 示例项目。
...
6. 支持两种标记不需要 token 的请求方式：`token: false` 和白名单 URL 列表
7. 三处拦截处理：
...
1. **实例级管理器**：在 `AxiosRequest` 实例化时创建，整个实例生命周期内复用
2. **私有级管理器**：在实例中按配置缓存，相同配置复用同一实例，不同配置创建新实例
3. **缓存 key**：基于配置内容生成 hash，确保相同配置复用
...
1. **Token 管理器**: 组件卸载时清理队列，防止内存泄漏
2. **Dedupe 管理器**: 超时自动清理 pendingRequests Map
3. **Cancel 管理器**: AbortController 取消后自动清理
4. **Retry 管理器**: 组件卸载时取消进行中的重试定时器
...
├── src/
│   ├── core/
│   │   └── AxiosRequest.ts          [修改] 核心类，重构管理器合并逻辑
│   ├── managers/
│   │   ├── TokenManager.ts           [修改] 增强：白名单、三处拦截、资源管理
│   │   ├── RetryManager.ts           [修改] 增强：配置简化、资源管理
│   │   ├── DedupeManager.ts          [修改] 增强：配置简化、资源管理
│   │   ├── CancelManager.ts          [修改] 增强：配置简化、资源管理
│   │   └── ManagerFactory.ts         [新增] 管理器工厂：创建和缓存私有级管理器
│   ├── types/
│   │   └── index.ts                  [修改] 增强类型定义，导出所有上下文类型
│   ├── utils/
│   │   ├── requestKey.ts             [保持] 保持不变
│   │   └── configMerger.ts           [新增] 配置合并工具函数
│   └── index.ts                      [修改] 导出新增类型和方法
...
│   │   ├── App.vue
│   │   ├── styles/
│   │   │   └── global.css           [新增] 全局样式，技术感主题
│   │   ├── types/
│   │   │   └── api.ts               [新增] API 类型定义
│   │   ├── utils/
│   │   │   └── request.ts           [新增] axios-request 实例化配置
│   │   ├── mock/
│   │   │   └── index.ts             [新增] Mock.js 模拟接口
│   │   ├── router/
│   │   │   └── index.ts             [新增] 路由配置
│   │   ├── layouts/
│   │   │   └── MainLayout.vue       [新增] 主布局组件
│   │   ├── components/
│   │   │   ├── AppHeader.vue        [新增] 顶部导航
│   │   │   └── TestPanel.vue        [新增] 测试面板组件
│   │   └── views/
│   │       ├── HomeView.vue          [新增] 首页/综合测试
│   │       ├── TokenView.vue         [新增] Token 管理器测试页
│   │       ├── RetryView.vue         [新增] 重试管理器测试页
│   │       ├── DedupeView.vue        [新增] 防重复提交测试页
│   │       ├── CancelView.vue        [新增] 取消请求测试页
│   │       └── CombinedView.vue     [新增] 多管理器协同测试页
│   └── public/
├── __tests__/                        [修改] 更新测试用例
...
export interface TokenManagerConfig {
  // 必填：判断 token 是否过期（基于错误对象）
  isTokenExpired: (error: any) => boolean;
  // 可选：判断响应是否表示 token 过期（基于响应数据）
  ...
  // 新增：白名单 URL（不需要 token 的请求）
  whitelistUrls?: (string | RegExp)[];
}

// 配置合并工具函数签名
export function mergeConfig<T>(defaultConfig: T, override?: Partial<T>): T;
```
