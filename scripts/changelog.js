...
function main() {
  ...
  if (fs.existsSync(changelogPath)) {
    ...
    // 移除旧的自动生成部分，保留手动部分
    const autoIndex = existingContent.indexOf('<!-- AUTO_GENERATED -->');
    if (autoIndex !== -1) {
      existingContent = existingContent.substring(0, autoIndex).trim();
      // 保留旧的自动生成部分
      autoGeneratedContent = existingContent.substring(autoIndex + '<!-- AUTO_GENERATED -->'.length).trim();
    }
  }
  
  // 写入新的 changelog（新版本插入到最前面）
